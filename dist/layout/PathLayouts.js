...
export class PathLayouts {
     ...
     */
    static getPathPoint(t, pathType, sphereU = 0.25, sphereV = 0.25, planeX = 0.5, planeZ = 0.5, spiralConfig = null, lineDirection = null, spiralAxis = null, radius = 2.0) {
     ...
     */
    static getStandardTangent(t, pathType, sphereU = 0.25, sphereV = 0.25, spiralConfig = null, spiralAxis = null) {
    ...
    static getStandardNormal(t, pathType, sphereU = 0.25, sphereV = 0.25, spiralConfig = null, spiralAxis = null) {
        if (pathType.startsWith('sphere')) {
    ...
    static getStandardBinormal(t, pathType, sphereU = 0.25, sphereV = 0.25, _planeX = 0.5, _planeZ = 0.5, spiralConfig = null, spiralAxis = null) {
        const tangent = PathLayouts.getStandardTangent(t, pathType, sphereU, sphereV, spiralConfig, spiralAxis);
     ...
     */
    static getStandardPathPoint(t, pathType, sphereU = 0.25, sphereV = 0.25, spiralConfig = null, _spiralAxis = null) {
        switch (pathType) {
            case 'line':
                return {
                    x: 0,
    ...
    static getTangent(t, pathType, sphereU = 0.25, sphereV = 0.25, spiralConfig = null, lineDirection = null, spiralAxis = null) {
        if (pathType.startsWith('sphere')) {
            return PathLayouts.getSphereUTangent(sphereU, sphereV);
        }
        if (pathType.startsWith('plane')) {
    ...
    static getNormal(t, pathType, sphereU = 0.25, sphereV = 0.25, spiralConfig = null, lineDirection = null, spiralAxis = null) {
        if (pathType.startsWith('sphere')) {
            return PathLayouts.getSphereVTangent(sphereU, sphereV);
        }
        if (pathType.startsWith('plane')) {
    ...
    static getBinormal(t, pathType, sphereU = 0.25, sphereV = 0.25, planeX = 0.5, planeZ = 0.5, spiralConfig = null, lineDirection = null, spiralAxis = null) {
        if (pathType.startsWith('sphere')) {
    ...
    static getSphereUTangent(sphereU, sphereV) {
        const phi = sphereU * Math.PI * 2;
        const theta = sphereV * Math.PI;
        const radius = 2;
        return vec3Normalize({
    ...
    static getSphereVTangent(sphereU, sphereV) {
        const phi = sphereU * Math.PI * 2;
        const theta = sphereV * Math.PI;
        const radius = 2;
        return vec3Normalize({
    ...
    static getPlaneXTangent() {
        return { x: 1, y: 0, z: 0 };
    }
    static getPlaneZTangent() {
        const tiltAngle = Math.PI / 4;
        return vec3Normalize({
            x: 0,
    ...
    static getCylinderVTangent() {
        return { x: 0, y: 1, z: 0 };
    }
    // --- Helper functions for getting tangents/normals at specific positions ---
    static getTangentAtPosition(position, pathType) {
        if (pathType.startsWith('sphere')) {
            const radius = 2;
    ...
    static getNormalAtPosition(position, pathType) {
        if (pathType.startsWith('sphere')) {
            const radius = 2;
            const phi = Math.atan2(position.z, position.x);
            const theta = Math.acos(position.y / radius);
    ...
    static getBinormalAtPosition(position, pathType) {
        if (pathType.startsWith('sphere')) {
            return vec3Normalize(position);
        }
        else if (pathType.startsWith('plane')) {
