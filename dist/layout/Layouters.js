...
export class Layouters {
    ...
    static circle(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, radius = 5.0, layoutDirection = undefined, lineDirection = undefined } = config;
        const direction = layoutDirection || lineDirection || null;
        const positions = PositionLayouts.getLayoutPositions('circle', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2, null, direction, null, radius);
        const transforms = [];
     ...
     */
    static line(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, layoutDirection = undefined, lineDirection = undefined } = config;
        const direction = layoutDirection || lineDirection || null;
        const positions = PositionLayouts.getLayoutPositions('line', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2, null, direction, null);
        const transforms = [];
        for (let i = 0; i < objectCount; i++) {
            const transform = LayoutUtils.calculateCompleteTransform({
                pathType: 'line',
     ...
     */
    static spiral(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, spiralConfig, layoutDirection = undefined, lineDirection = undefined } = config;
        const direction = layoutDirection || lineDirection || null;
        const positions = PositionLayouts.getLayoutPositions('spiral', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2, spiralConfig ?? null, direction, direction);
        const transforms = [];
        for (let i = 0; i < objectCount; i++) {
            const transform = LayoutUtils.calculateCompleteTransform({
                pathType: 'spiral',
     ...
     * Sphere Grid layout.
     */
    static sphereGrid(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, radius = 8.0 } = config;
        const positions = PositionLayouts.getLayoutPositions('sphere_grid', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2, null, null, null, radius);
     ...
     */
    static sphereHex(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, radius = 8.0 } = config;
        const positions = PositionLayouts.getLayoutPositions('sphere_hex', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2, null, null, null, radius);
        if (!positions || positions.length === 0) {
     ...
     */
    static sphereHexHoneycomb(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, radius = 8.0 } = config;
     ...
     * Plane Grid layout.
     */
    static planeGrid(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false } = config;
        const positions = PositionLayouts.getLayoutPositions('plane_grid', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2);
        if (!positions || positions.length === 0) {
     ...
     * Plane Hex layout.
     */
    static planeHex(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false } = config;
        const positions = PositionLayouts.getLayoutPositions('plane_hex', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2);
        if (!positions || positions.length === 0) {
     ...
     */
    static cylinderGrid(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false } = config;
        const positions = PositionLayouts.getLayoutPositions('cylinder_grid', objectCount, spacing, spacingMode, 0, 0.25, 0.25, 0.5, 0.5, 'auto', 3, 3, 'radial', 2);
        if (!positions || positions.length === 0) {
     ...
     */
    static _adaptiveLineLayoutPositionsOnly(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, objects, layoutDirection = undefined, lineDirection = undefined } = config;
        const direction = layoutDirection || lineDirection || null;
        ...
        if (lineLength < 0.001) {
            ...
            const directionConfig = { ...config, spacing: 0.1 };
     ...
     */
    static _adaptiveLineLayoutWithDirection(config, startPos, unitDirection, lineLength) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, objects } = config;
        const finalWidths = [];
        for (let i = 0; i < objectCount && objects && i < objects.length; i++) {
            const obj = objects[i];
            const naturalWidth = obj.userData.naturalWidth || 0.3;
     ...
     */
    static _adaptiveCircleLayoutPositionsOnly(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, objects, radius = 5.0, layoutDirection = undefined, lineDirection = undefined } = config;
        const direction = layoutDirection || lineDirection || null;
        ...
        let circleNormal, circleU, circleV;
        if (direction) {
        ...
        for (let i = 0; i < objectCount && objects && i < objects.length && i < baseTransforms.length; i++) {
            ...
            const { tangent, normal, binormal } = LayoutUtils.getSurfaceVectors('circle', newPosition, tParameter, null, null, null, null, null, direction, null);
            const rotation = LayoutUtils.calculateOrientation(tangent, normal, binormal, newPosition, orientation, upMode, 'circle', flip180);
            adaptiveTransforms.push({
                position: newPosition,
     ...
     */
    static _adaptiveSpiralLayoutPositionsOnly(config) {
        const { objectCount, spacingMode = 'distribute', spacing = 1.5, orientation = 'aligned', upMode = 'world', flip180 = false, objects, spiralConfig, layoutDirection = undefined, lineDirection = undefined } = config;
        const direction = layoutDirection || lineDirection || null;
        ...
        let spiralAxisVector, spiralU, spiralV;
        if (direction) {
        ...
        for (let i = 0; i < objectCount && objects && i < objects.length && i < baseTransforms.length; i++) {
            ...
            if (i < objectCount - 1) {
                ...
                const tIncrement = Layouters._findSpiralTIncrementWithDirection(currentT, spacingDistance, spiralTurns, maxRadius, growthRate, spiralAxisVector, spiralU, spiralV);
                currentT += tIncrement;
            }
            const { tangent, normal, binormal } = LayoutUtils.getSurfaceVectors('spiral', newPosition, currentT, null, null, null, null, { spiralTurns, maxRadius, growthRate, startDistance }, direction, direction);
            const rotation = LayoutUtils.calculateOrientation(tangent, normal, binormal, newPosition, orientation, upMode, 'spiral', flip180);
            adaptiveTransforms.push({
                position: newPosition,
                rotation: { x: rotation.x, y: rotation.y, z: rotation.z },
                scale: { x: 1, y: 1, z: 1 },
     ...
     */
    static findSpiralTIncrement(currentT, targetDistance, spiralTurns, maxRadius, growthRate = 2) {
     ...
     */
    static _findSpiralTIncrementWithDirection(currentT, targetDistance, spiralTurns, maxRadius, growthRate, spiralAxisVector, spiralU, spiralV) {
        let tIncrement = targetDistance / (maxRadius * 2 * Math.PI);
        const maxIterations = 10;
        const tolerance = 0.001;
        for (let iter = 0; iter < maxIterations; iter++) {
     ...
     */
    static _getSpiralPositionWithDirection(t, spiralTurns, maxRadius, growthRate, spiralAxisVector, spiralU, spiralV) {
        const angle = t * spiralTurns * 2 * Math.PI;
        const radius = (t / 1.0) * maxRadius;
