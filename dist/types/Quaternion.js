...
export class Quaternion {
    ...
    toEulerAnglesDegrees() {
        ...
        return new Vector3(eulerRad.x * radToDeg, eulerRad.y * radToDeg, eulerRad.z * radToDeg);
    }
    equals(q, tolerance = 0.000001) {
    ...
    static slerp(q1, q2, t) {
        ...
        if (dot > 0.9995) {
            const result = new Quaternion(q1.x + t * (q2Copy.x - q1.x), q1.y + t * (q2Copy.y - q1.y), q1.z + t * (q2Copy.z - q1.z), q1.w + t * (q2Copy.w - q1.w));
        ...
        return new Quaternion(s0 * q1.x + s1 * q2Copy.x, s0 * q1.y + s1 * q2Copy.y, s0 * q1.z + s1 * q2Copy.z, s0 * q1.w + s1 * q2Copy.w);
    }
    static fromEulerAnglesDegrees(eulerDegrees) {
        const degToRad = Math.PI / 180;
        const q = new Quaternion();
        q.setFromEulerAngles(eulerDegrees.x * degToRad, eulerDegrees.y * degToRad, eulerDegrees.z * degToRad);
        return q;
    }
    slerp(target, alpha) {
