...
1. **Repository context**
   - Project lives at `/Volumes/DatenAP/Code/typescript-utils`.
...
4. **Deployment target**
   - Built files go to `/var/www/typescript-utils`.
   - Backups are stored with prefix `typescript-utils` under `/var/backups/` (configurable via templates).

5. **Startup checklist for new session**
   - `cd /Volumes/DatenAP/Code/typescript-utils`.
