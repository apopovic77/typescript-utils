...
6. Validate the site at `https://example.com` (if applicable) and confirm fresh assets reached `/var/www/typescript-utils`.

## Rollback Strategy
- Use `.devops/rollback.sh` on the server to restore from `/var/backups/typescript-utils-*`.
